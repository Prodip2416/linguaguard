...
/** Flat key-value map for a single language: { "key.subkey": "value" } */
export type TranslationMap = Record<string, string>;

/** All languages loaded: { "en": { ... }, "fr": { ... } } */
export type LocaleData = Record<string, TranslationMap>;
...
export function readLocaleFiles(
  ...
  if (!fs.existsSync(resolvedPath)) {
    throw new Error(
      `Locales directory not found: "${resolvedPath}"\n` +
  ...
  for (const lang of languages) {
    ...
    try {
      ...
      if (err instanceof SyntaxError) {
        throw new Error(
          `Invalid JSON in locale file: ${filePath}\n` +
            `JSON parse error: ${err.message}`
        );
      }
      throw new Error(
        `Failed to read locale file: ${filePath}\n${String(err)}`
      );
    }
  }
 ...
 */
export function getAllKeys(localeData: LocaleData): Set<string> {
...
export function getPrimaryKeys(
  ...
  if (!primary) {
    throw new Error(
      `Primary language "${primaryLang}" not found in loaded locale data.\n` +
