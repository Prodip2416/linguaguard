...
import { readLocaleFiles, getPrimaryKeys, LocaleData } from "../utils/fileReader.js";

// ─── Types ────────────────────────────────────────────────────────────────────

export interface MissingKeysResult {
  /** Summary sentence shown at the top of the report */
  summary: string;
  /** Per-language breakdown of missing keys */
  details: LanguageMissingKeys[];
  /** Total count of missing key occurrences across all languages */
  totalMissing: number;
}

export interface LanguageMissingKeys {
  language: string;
  missingKeys: string[];
  count: number;
}
...
export function findMissingKeys(
  ...
  const localeData: LocaleData = readLocaleFiles(localesPath, languages, extension);

  // Get the full set of keys from the primary language
  const primaryKeys = getPrimaryKeys(localeData, primaryLang);

  const details: LanguageMissingKeys[] = [];
  let totalMissing = 0;
 ...
 */
export function runMissingKeysTool(args: Record<string, unknown>): string {
 ...
 */
function formatMissingKeysReport(result: MissingKeysResult): string {
  const lines: string[] = [];
