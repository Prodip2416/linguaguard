...
const CommonInput = z.object({
    locales_path: z.string().optional().describe("Path to locale files folder. Defaults to LOCALES_PATH env var (e.g. ./src/locales)"),
    languages: z.string().optional().describe("Comma-separated language codes. Defaults to LANGUAGES env var (e.g. en,fr,bn,ar)"),
    primary_lang: z.string().optional().describe("Primary/reference language code. Defaults to PRIMARY_LANG env var (e.g. en)"),
    file_extension: z.string().optional().describe("File extension without dot. Defaults to FILE_EXTENSION env var (e.g. json)"),
});
...
server.registerTool("find_unused_keys", {
    ...
    inputSchema: CommonInput.extend({
        project_root: z.string().optional().describe("Root directory of the frontend project to scan for key usage. Defaults to PROJECT_ROOT env var or current directory."),
    }),
}, async (args) => {
...
server.registerTool("sync_key", {
    ...
    inputSchema: CommonInput.extend({
        key: z.string().describe("The translation key to add, in dot notation (e.g. auth.forgot_password or common.submit_button)"),
        value: z.string().describe("The translation value for the primary language (e.g. 'Forgot your password?')"),
        overwrite: z.boolean().optional().describe("Set to true to overwrite the key if it already exists in a language file. Default: false."),
    }),
}, async (args) => {
...
server.registerTool("check_naming_convention", {
    ...
    inputSchema: CommonInput.extend({
        naming_convention: z.enum(["snake_case", "camelCase", "kebab-case", "PascalCase"]).optional().describe("Naming convention to enforce. Defaults to NAMING_CONVENTION env var (e.g. snake_case)"),
    }),
}, async (args) => {
...
server.registerTool("i18n_health_report", {
    ...
    inputSchema: CommonInput.extend({
        naming_convention: z.enum(["snake_case", "camelCase", "kebab-case", "PascalCase"]).optional().describe("Naming convention to check against. Defaults to NAMING_CONVENTION env var."),
        project_root: z.string().optional().describe("Root directory to scan for key usage. Defaults to PROJECT_ROOT env var or '.'."),
    }),
}, async (args) => {
...
server.registerTool("suggest_translations", {
    ...
    inputSchema: CommonInput.extend({
        max_keys: z.number().optional().describe("Maximum number of missing keys to translate per language in one call. Default: 20. Lower this if responses are too slow."),
    }),
}, async (args) => {
