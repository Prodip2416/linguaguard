 ...
 *   trans("button.cancel")
 */
const I18N_KEY_PATTERN = /(?:\$t|i18n\.t|trans|translate|t)\s*\(\s*['"`]([a-zA-Z0-9_.:-]+)['"`]/g;
// ─── Main function ────────────────────────────────────────────────────────────
/**
 ...
 */
export async function scanCodeForKeys(projectRoot) {
    const resolvedRoot = path.resolve(projectRoot);
    // Check the project root exists
    if (!fs.existsSync(resolvedRoot)) {
        throw new Error(`Project root directory not found: "${resolvedRoot}"\n` +
            `Make sure the path passed to scanCodeForKeys is correct.`);
    }
    // Find all relevant source files — JS, TS, JSX, TSX, Vue, Svelte
    const files = await glob(`${resolvedRoot}/**/*.{js,ts,jsx,tsx,vue,svelte}`, {
        ignore: [
            "**/node_modules/**",
            "**/dist/**",
            "**/build/**",
 ...
 */
export async function getUsedKeys(projectRoot) {
 ...
 */
export async function countScannedFiles(projectRoot) {
    const resolvedRoot = path.resolve(projectRoot);
    if (!fs.existsSync(resolvedRoot))
        return 0;
    const files = await glob(`${resolvedRoot}/**/*.{js,ts,jsx,tsx,vue,svelte}`, {
