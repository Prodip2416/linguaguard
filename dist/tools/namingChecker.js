 ...
 */
export function checkNamingConvention(localesPath, languages, convention, extension = "json") {
...
// ─── MCP tool handler ─────────────────────────────────────────────────────────
export function runNamingCheckerTool(args) {
    ...
    const rawConvention = args.naming_convention ||
    ...
    if (!validConventions.includes(rawConvention)) {
        return (`Error: Invalid naming_convention "${rawConvention}". ` +
            `Valid options: ${validConventions.join(", ")}`);
    }
    const convention = rawConvention;
    try {
...
// ─── Report formatter ─────────────────────────────────────────────────────────
function formatNamingCheckReport(result) {
    const lines = [];
    ...
    if (result.violations.length > 0) {
        ...
        lines.push("");
        lines.push("| Key | Violating Segment | Expected Convention |");
        lines.push("|-----|-------------------|---------------------|");
        for (const v of result.violations) {
            lines.push(`| \`${v.key}\` | \`${v.violatingSegment}\` | ${v.expected} |`);
        }
    }
    return lines.join("\n");
}
//# sourceMappingURL=namingChecker.js.map