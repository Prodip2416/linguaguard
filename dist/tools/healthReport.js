...
import { checkNamingConvention } from "./namingChecker.js";
// ─── Core logic ───────────────────────────────────────────────────────────────
/**
 * Runs all health checks and combines them into a single report.
 */
export async function generateHealthReport(localesPath, languages, primaryLang, extension = "json", convention = "snake_case", projectRoot = ".") {
    // 1. Load locale data
    ...
    for (const lang of languages) {
        ...
        const completenessPercent = primaryKeyCount === 0
            ? 100
    ...
    try {
        const namingResult = checkNamingConvention(localesPath, languages, convention, extension);
    ...
    const nonPrimaryLangs = languageHealth.filter((l) => l.language !== primaryLang);
    const avgCompleteness = nonPrimaryLangs.length === 0
        ? 100
    ...
    const namingPenalty = namingViolationCount < 0 ? 0 : Math.min(namingViolationCount, 10);
    const overallScore = Math.max(0, Math.round(avgCompleteness - unusedPenalty - namingPenalty));
    // 6. Summary sentence
...
// ─── MCP tool handler ─────────────────────────────────────────────────────────
export async function runHealthReportTool(args) {
    ...
    const convention = (args.naming_convention ||
        process.env.NAMING_CONVENTION ||
        "snake_case");
    const projectRoot = args.project_root || process.env.PROJECT_ROOT || ".";
    try {
        const result = await generateHealthReport(localesPath, languages, primaryLang, extension, convention, projectRoot);
...
function formatHealthReport(result) {
    ...
    lines.push(`- Naming violations:  ${result.namingViolationCount < 0 ? "N/A" : result.namingViolationCount}`);
    ...
    for (const lang of result.languageHealth) {
        ...
        lines.push(`| \`${lang.language}\` | ${lang.presentKeys}/${lang.totalKeys} | ${lang.missingKeys} | ${bar} ${lang.completenessPercent}% |`);
    }
    // Recommendations
    const recommendations = [];
    const incompleteLangs = result.languageHealth.filter((l) => l.completenessPercent < 100);
    if (incompleteLangs.length > 0) {
        recommendations.push(`Run \`find_missing_keys\` to see exactly which keys are missing in: ${incompleteLangs
            .map((l) => l.language)
            .join(", ")}`);
    }
    if (result.unusedKeyCount > 0) {
        recommendations.push(`Run \`find_unused_keys\` to review ${result.unusedKeyCount} potentially dead key(s).`);
    }
    if (result.namingViolationCount > 0) {
        recommendations.push(`Run \`check_naming_convention\` to see ${result.namingViolationCount} naming violation(s).`);
    }
