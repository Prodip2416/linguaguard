 ...
 */
export function runCiGuardCheck(localesPath, languages, primaryLang, extension = "json") {
 ...
 */
export function runCiGuardTool(args) {
...
// ─── Report formatter ─────────────────────────────────────────────────────────
function formatCiGuardReport(result) {
    const lines = [];
    ...
    if (result.details.length > 0) {
        ...
        lines.push("");
        lines.push("---");
        lines.push(`**Action required:** Add the missing keys to the affected language files ` +
            `or use the \`sync_key\` tool to propagate them. ` +
            `This check would exit with code **1** in a CI pipeline.`);
    }
    else {
        lines.push("");
        lines.push("All language files are fully translated. This check exits with code **0**.");
    }
    return lines.join("\n");
}
//# sourceMappingURL=ciGuard.js.map