 ...
 */
export async function suggestTranslations(localesPath, languages, primaryLang, extension = "json", maxKeys = 20) {
    // 1. Check for API key
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) {
        throw new Error("ANTHROPIC_API_KEY is not set. Add it to your MCP server env config to use AI translation.");
    }
...
Return the ${langName} translations as a JSON object with the same keys:`;
        try {
            ...
            console.warn(`[LinguaGuard] Failed to get translations for "${lang}": ${String(err)}`);
            suggestions.push({
                key: "_error",
                language: lang,
...
// ─── MCP tool handler ─────────────────────────────────────────────────────────
export async function runAiTranslationTool(args) {
    ...
    try {
        const result = await suggestTranslations(localesPath, languages, primaryLang, extension, maxKeys);
...
// ─── Report formatter ─────────────────────────────────────────────────────────
function formatAiTranslationReport(result) {
    const lines = [];
    ...
    lines.push("");
    lines.push("---");
    lines.push("> These are AI-generated suggestions. Review before using. " +
        "Use the `sync_key` tool to apply individual translations.");
    return lines.join("\n");
}
//# sourceMappingURL=aiTranslation.js.map